...
function loadServerInfo(): {
  ...
  const entrypointDir = dirname(
    process.argv[1] ? resolve(process.argv[1]) : process.cwd()
  );
  const packageJsonPath = resolve(entrypointDir, "..", "..", "package.json");
  ...
  const buildInfoPath = resolve(entrypointDir, "..", "build-info.json");
