...
type CurrentRow={replay_sha256:unknown;analysis_key:unknown;analysis_id:unknown;played_at_unix_s:unknown;map_name:unknown;
  owner:unknown;observed_name:unknown;race:unknown;self_key:unknown;self_display:unknown;replay_manifest_path:unknown};
...
async function currentReportRows(dbPath:string,root:string):Promise<{rows:StaticReportRow[];warnings:StaticReportIndexResult["warnings"]}>{
  ...
  try{
    ...
    if(identities)for(const row of sqlRows(db,`SELECT c.player_key,a.alias_name FROM canonical_players c
      JOIN player_aliases a USING(player_id) ORDER BY c.player_key,a.name_namespace,a.observed_name_key`)){
      const key=String(row.player_key),names=knownNames.get(key)??[],name=String(row.alias_name);
      if(!names.includes(name))names.push(name);knownNames.set(key,names);
    }
  }finally{db.close();}
  ...
  for(const [replaySha256,group] of grouped){
    ...
    const participants=group.filter(row=>row.owner!==null).map(row=>({owner:Number(row.owner),
      name:row.self_display==null?String(row.observed_name):String(row.self_display),observedName:String(row.observed_name),
      knownNames:row.self_key==null?[]:[...(knownNames.get(String(row.self_key))??[])],race:String(row.race)}));
...
export function filterReportRows(rows:StaticReportRow[],filters:{search?:string;player?:string;map?:string;year?:string;racePair?:string}):StaticReportRow[]{
  ...
  return rows.filter(row=>(!search||[row.replaySha256,row.map,row.racePair,...row.participants.flatMap(p=>[p.name,p.observedName,...p.knownNames])].join("\n").toLocaleLowerCase().includes(search))&&
