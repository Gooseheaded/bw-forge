...
def enqueue(db, args):
    ...
    try:
        db.execute('''INSERT INTO replays(sha256,byte_size,raw_relative_path,first_seen_at_ms,map_name,played_at_unix_s)
            VALUES (?,?,?,?,?,?) ON CONFLICT(sha256) DO NOTHING''', (sha,size,rel,now,map_name,played_at))
        replay = db.execute('SELECT replay_id,byte_size,raw_relative_path,map_name,played_at_unix_s FROM replays WHERE sha256=?', (sha,)).fetchone()
        ...
        if (replay['map_name'] is None or not replay['map_name'].strip()) and map_name is not None:
            db.execute("UPDATE replays SET map_name=? WHERE replay_id=? AND (map_name IS NULL OR trim(map_name)='')", (map_name,replay['replay_id']))
