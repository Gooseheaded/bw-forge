...
generation-bearing `bw_unit_instance_id(index) -> i32` export. Release v0.1.2
also exposes the engine's existing five-slot production queue and normalizes
affected SCR replay-local UnitIds during REP-to-DRPL conversion through the
upstream JavaScript API. The original Wasm is retained for provenance only.
