...
use crate::app_messages::{GameSetupInfo, MapInfo, ReplayProgress, SbUserId, StartingFog};
...
pub unsafe fn order_return_minerals(
    bw: &BwScr,
    unit: *mut bw::Unit,
    orig: unsafe extern "C" fn(*mut bw::Unit),
) {
    order_return_resources(bw, unit, orig, ResourceKind::Minerals);
}

pub unsafe fn order_return_gas(
    bw: &BwScr,
    unit: *mut bw::Unit,
    orig: unsafe extern "C" fn(*mut bw::Unit),
) {
    order_return_resources(bw, unit, orig, ResourceKind::Gas);
}
...
unsafe fn order_return_resources(
    bw: &BwScr,
    unit: *mut bw::Unit,
    orig: unsafe extern "C" fn(*mut bw::Unit),
    kind: ResourceKind,
) {
    let before = Unit::from_ptr(unit).map(|unit| ResourceReturnState {
        owner: unit.player(),
        carrying: match kind {
            ResourceKind::Minerals => unit.is_carrying_minerals(),
            ResourceKind::Gas => unit.is_carrying_gas(),
        },
        carried_amount: unit.carried_resource_amount() as u32,
    });
    orig(unit);

    let Some(before) = before.filter(|state| state.carrying && state.carried_amount != 0) else {
        return;
    };

    let after_amount = Unit::from_ptr(unit)
        .map(|unit| unit.carried_resource_amount() as u32)
        .unwrap_or(0);
    let gathered = before.carried_amount.saturating_sub(after_amount);
    if gathered == 0 {
        return;
    }

    match kind {
        ResourceKind::Minerals => bw.record_minerals_gathered(before.owner, gathered),
        ResourceKind::Gas => bw.record_gas_gathered(before.owner, gathered),
    }
}

#[derive(Clone, Copy)]
struct ResourceReturnState {
    owner: u8,
    carrying: bool,
    carried_amount: u32,
}
