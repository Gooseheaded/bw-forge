...
fn show_window(window: HWND, show: i32, orig: unsafe extern "C" fn(HWND, i32) -> u32) -> u32 {
    unsafe {
        ...
        if show != SW_HIDE && is_forge_window(window) && replay_export::is_enabled() {
            debug!("Suppressing replay-export window visibility request {show}");
            return orig(window, SW_HIDE);
        }
        if show == SW_SHOW
