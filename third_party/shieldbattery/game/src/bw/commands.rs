...
const MAX_REPLAY_SPEED_MULTIPLIER: u32 = 1024;
        ...
        let ok = match command {
            ...
            [id::REPLAY_SPEED, rest @ ..] => {
                if from_replay || rest.len() != 9 {
                    ...
                    pause < 2 && speed < 7 && valid_replay_speed_multiplier(multiplier)
                }
            }
            _ => {
                if is_observer {
...
fn valid_replay_speed_multiplier(multiplier: u32) -> bool {
    multiplier != 0 && multiplier <= MAX_REPLAY_SPEED_MULTIPLIER && multiplier.is_power_of_two()
}
...
mod test {
    ...
    fn filter_replay_speed() {
        ...
        // Ok
        let data9 = &[
            0x20, 0xff, 0xff, 0x56, 0x00, 0x06, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x62,
            0xff, 0xff, 0xff, 0xff,
        ];
        // Invalid multiplier
        let data10 = &[
            0x20, 0xff, 0xff, 0x56, 0x00, 0x06, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x62,
            0xff, 0xff, 0xff, 0xff,
        ];
        ...
        assert_eq!(
            &*filter_invalid_commands(data9, false, false, LENGTHS),
            data9
        );
        assert_eq!(
            &*filter_invalid_commands(data10, false, false, LENGTHS),
            expected_bad
        );
    }
