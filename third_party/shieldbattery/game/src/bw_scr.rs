...
use crate::app_messages::{
    AtomicStartingFog, MapInfo, ReplayProgress, SbUserId, Settings, StartingFog,
};
...
use crate::replay_export;
use crate::snp;
use crate::sync::DumbSpinLock;
use crate::tick_hash;
use crate::unit_timeline;
use crate::windows;
...
const MAX_REPLAY_EXPORT_FRAMES: u32 = 30 * 60 * 24;
...
pub struct BwScr {
    ...
    order_return_minerals: VirtualAddress,
    order_return_gas: VirtualAddress,
    ...
    replay_export_speed_applied: AtomicBool,
    next_replay_export_progress_frame: AtomicU32,
    last_replay_export_progress_frame: AtomicU32,
    replay_export_seen_connected_players: AtomicBool,
    replay_export_cutoff_frame: AtomicU32,
    tick_hash: tick_hash::State,
    unit_timeline: unit_timeline::State,
...
impl BwScr {
    ...
    pub fn new() -> Result<BwScr, BwInitError> {
        ...
        let order_return_minerals = analysis
            .order_function(order::RETURN_MINERALS.0)
            .ok_or("order_return_minerals")?;
        let order_return_gas = analysis
            .order_function(order::RETURN_GAS.0)
            .ok_or("order_return_gas")?;
        ...
        Ok(BwScr {
            ...
            replay_export_speed_applied: AtomicBool::new(false),
            next_replay_export_progress_frame: AtomicU32::new(0),
            last_replay_export_progress_frame: AtomicU32::new(u32::MAX),
            replay_export_seen_connected_players: AtomicBool::new(false),
            replay_export_cutoff_frame: AtomicU32::new(u32::MAX),
            tick_hash: tick_hash::State::from_environment(),
            unit_timeline: unit_timeline::State::from_environment(),
    ...
    pub unsafe fn patch_game(&'static self, image: *mut u8) {
        unsafe {
            ...
            let address = self.order_return_minerals.0 as usize - base;
            exe.hook_closure_address(
                OrderFn,
                |unit, orig| {
                    game_thread::order_return_minerals(self, unit, orig);
                },
                address,
            );
            let address = self.order_return_gas.0 as usize - base;
            exe.hook_closure_address(
                OrderFn,
                |unit, orig| {
                    game_thread::order_return_gas(self, unit, orig);
                },
                address,
            );
        ...
        unsafe {
            ...
            exe.hook_closure_address(
                ...
                move |extra_funcs, extra_func_len, second_draw, orig| {
                    ...
                    if replay_export::disable_render() && is_replay {
                        return;
                    }
    ...
    fn reset_state_for_game_init(&self) {
        ...
        self.replay_export_speed_applied
            .store(false, Ordering::Relaxed);
        self.next_replay_export_progress_frame
            .store(0, Ordering::Relaxed);
        self.last_replay_export_progress_frame
    ...
    pub fn unit_array(&self) -> bw_dat::UnitArray {
        unsafe {
            let units = self.units.resolve();
            bw_dat::UnitArray::new((*units).data as *mut bw::Unit, (*units).length as usize)
        }
    }
    ...
    pub fn flush_debug_traces(&self) {
        self.tick_hash.flush();
        self.unit_timeline.flush();
    }

    pub fn reset_timeline_tracking(&self) {
        self.unit_timeline.reset_for_game();
        self.replay_export_seen_connected_players
            .store(false, Ordering::Relaxed);
        self.replay_export_cutoff_frame
            .store(u32::MAX, Ordering::Relaxed);
        self.next_replay_export_progress_frame
            .store(0, Ordering::Relaxed);
        self.last_replay_export_progress_frame
    ...
    pub fn record_minerals_gathered(&self, owner: u8, amount: u32) {
        self.unit_timeline.record_minerals_gathered(owner, amount);
    }

    pub fn record_gas_gathered(&self, owner: u8, amount: u32) {
        self.unit_timeline.record_gas_gathered(owner, amount);
    }

    fn replay_export_effective_total_frames(&self) -> u32 {
        unsafe {
            let replay_header = self.replay_header();
            let header_total = if replay_header.is_null() {
                0
            } else {
                (*replay_header).replay_end_frame
            };
            let mut total = if header_total == 0 {
                MAX_REPLAY_EXPORT_FRAMES
            } else {
                header_total.min(MAX_REPLAY_EXPORT_FRAMES)
            };
            let cutoff = self.replay_export_cutoff_frame.load(Ordering::Relaxed);
            if cutoff != u32::MAX {
                total = total.min(cutoff);
            }
            total
        }
    }

    fn should_emit_replay_export_traces(&self, frame: u32) -> bool {
        let cutoff = self.replay_export_cutoff_frame.load(Ordering::Relaxed);
        cutoff == u32::MAX || frame <= cutoff
    }

    fn update_replay_export_cutoff(&self) {
        if !replay_export::is_enabled() || !game_thread::is_replay() {
            return;
        }

        unsafe {
            let frame = (*self.game()).frame_count;
            let mut cutoff = self.replay_export_cutoff_frame.load(Ordering::Relaxed);
            if cutoff != u32::MAX {
                return;
            }

            if frame >= MAX_REPLAY_EXPORT_FRAMES {
                self.replay_export_cutoff_frame
                    .store(MAX_REPLAY_EXPORT_FRAMES, Ordering::Relaxed);
                return;
            }

            let players = self.players();
            let storm_flags = self.storm_player_flags();
            let mut any_real_player_slots = false;
            let mut any_connected_real_players = false;
            for i in 0..8usize {
                let player = players.add(i);
                if (*player).player_type != bw::PLAYER_TYPE_HUMAN {
                    continue;
                }
                any_real_player_slots = true;
                let storm_id = (*player).storm_id as usize;
                if storm_id < storm_flags.len() && storm_flags[storm_id] != 0 {
                    any_connected_real_players = true;
                    break;
                }
            }

            if any_connected_real_players {
                self.replay_export_seen_connected_players
                    .store(true, Ordering::Relaxed);
                return;
            }

            let seen_connected = self
                .replay_export_seen_connected_players
                .load(Ordering::Relaxed);
            if any_real_player_slots && seen_connected {
                cutoff = frame;
                self.replay_export_cutoff_frame
    ...
    fn apply_replay_export_speed(&self) {
        if !replay_export::is_enabled() || !game_thread::is_replay() {
            return;
        }
        if self
            .replay_export_speed_applied
        ...
        command[0] = commands::id::REPLAY_SPEED;
        command[1] = 0;
        LittleEndian::write_u32(&mut command[2..6], 6);
        LittleEndian::write_u32(&mut command[6..10], replay_export::target_multiplier());
        unsafe {
            (self.send_command)(command.as_ptr(), command.len());
        }
    }

    fn emit_replay_export_progress(&self) {
        if !replay_export::is_enabled() || !game_thread::is_replay() {
            return;
        }

        const PROGRESS_INTERVAL_FRAMES: u32 = 24 * 5;

        unsafe {
            let game = self.game();
            let replay_header = self.replay_header();
            if game.is_null() || replay_header.is_null() {
                return;
            }

            let frame = (*game).frame_count;
            let total_frames = self.replay_export_effective_total_frames();
            let last_frame = self
                .last_replay_export_progress_frame
                .load(Ordering::Relaxed);
            if frame == last_frame {
                return;
            }
            let next_frame = self
                .next_replay_export_progress_frame
                .load(Ordering::Relaxed);
            let is_final_frame = total_frames != 0 && frame >= total_frames;
            if frame < next_frame && !is_final_frame {
                return;
            }

            self.next_replay_export_progress_frame.store(
                frame.saturating_add(PROGRESS_INTERVAL_FRAMES),
                Ordering::Relaxed,
            );
            self.last_replay_export_progress_frame
                .store(frame, Ordering::Relaxed);
            send_game_msg_to_async(GameThreadMessage::ReplayProgress(ReplayProgress {
                frame,
                total_frames,
                time_ms: u64::from(frame).saturating_mul(42),
            }));
        }
    }
...
unsafe fn step_game_logic_hook(
    ...
    bw.update_replay_export_cutoff();
    let frame = (*bw.game()).frame_count;
    if bw.should_emit_replay_export_traces(frame) {
        bw.tick_hash.emit_if_enabled(bw);
        bw.tick_hash.emit_hidden_if_enabled(bw);
        bw.unit_timeline.emit_if_enabled(bw);
    }
    bw.apply_replay_export_speed();
    bw.emit_replay_export_progress();
    ret
}
