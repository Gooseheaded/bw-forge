...
export type ActiveGameManagerEvents = {
  ...
  replayProgress: [gameId: string, info: ReplayProgressInfo]
...
export class ActiveGameManager extends EventEmitter<ActiveGameManagerEvents> {
  ...
  async handleGameConnected(id: string) {
    ...
    const scr = applyReplayExportAudioSettings(
  ...
  handleReplayProgress(gameId: string, info: ReplayProgressInfo) {
    if (!this.activeGame || this.activeGame.id !== gameId) {
      return
    }

    this.emit('replayProgress', gameId, info)
  }
  ...
  handleGameExit(id: string, exitCode: number) {
    ...
    if (!this.activeGame.config?.setup.replayExport?.enabled) {
      Promise.resolve()
...
async function doLaunch(
  ...
  replayExport?: ReplayExportConfig,
) {
  ...
  await scrSettings.writeGameSettingsFile(replayExportAudioOverrides(replayExport))
