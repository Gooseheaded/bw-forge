...
const DEFAULT_UNIT_TIMELINE_JSONL_OUTPUT_PATH = String.raw`C:\Windows\Temp\sb-unit-timeline.jsonl`
const DEFAULT_UNIT_TIMELINE_MSGPACK_OUTPUT_PATH = String.raw`C:\Windows\Temp\sb-unit-timeline.sbtl`
const replayExportLaunchArgs = parseReplayExportLaunchArgs(process.argv.slice(1))
let replayExportRendererReady = false
let pendingReplayLaunchArgs: string[] | undefined
...
const appIdentity = resolveAppIdentity(app.name)
const modelId = appIdentity.appId
const updateUrl = appIdentity.updateUrl
setAppId(modelId)
...
export function notifyNewInstance(data: NewInstanceNotification) {
  if (mainWindow && shouldFocusWindowForLaunchArgs(data.args.slice(1))) {
...
function handleLaunchArgs(args: string[]) {
  ...
  const replayExport = parseReplayExportLaunchArgs(args)

  const replays = args
    .filter(arg => !arg.startsWith('--') && arg.toLowerCase().endsWith('.rep'))
    .map(p => path.resolve('.', p))
  if (replays.length) {
    if (replayExport.enabled) {
      if (!replayExportRendererReady) {
        pendingReplayLaunchArgs = args
        logReplayExport('renderer not ready yet, queueing replay launch')
        return
      }

      TypedIpcSender.from(mainWindow?.webContents).send('replaysOpenForExport', {
        replayPath: replays[0],
        replayExport: {
          enabled: true,
          targetMultiplier: replayExport.targetMultiplier ?? 128,
          disableRender: replayExport.disableRender ?? true,
          disableAudio: replayExport.disableAudio ?? true,
        },
      })
      logReplayExport(`sent replay export request for ${replays[0]}`)
    } else {
...
function logReplayExport(message: string) {
  logger.info(`[replay-export] ${message}`)
}

function formatReplayExportTime(timeMs: number): string {
...
function replayExportTimelineOutputPath(): string {
  const envPath = process.env.SB_UNIT_TIMELINE_OUT?.trim()
  if (envPath) {
    return envPath
  }

  const format = process.env.SB_UNIT_TIMELINE_FORMAT?.trim().toLowerCase()
  if (format === 'msgpack' || format === 'mpk' || format === 'mp') {
    return DEFAULT_UNIT_TIMELINE_MSGPACK_OUTPUT_PATH
  }

  return DEFAULT_UNIT_TIMELINE_JSONL_OUTPUT_PATH
}
...
function setupIpc(localSettings: LocalSettingsManager, scrSettings: ScrSettingsManager) {
  ipcMain.on('replayExportRendererReady', () => {
    replayExportRendererReady = true
    if (replayExportLaunchArgs.enabled) {
      logReplayExport('renderer ready')
    }
    if (pendingReplayLaunchArgs) {
      const args = pendingReplayLaunchArgs
      pendingReplayLaunchArgs = undefined
  ...
  if (!isDev && appIdentity.autoUpdate) {
  ...
  let replayExportQuitTimer: ReturnType<typeof setTimeout> | undefined
  let replayExportTerminalFrameTimer: ReturnType<typeof setTimeout> | undefined
  let replayExportProgressLogInFlight = false
  let replayExportPendingProgress:
    | { gameId: string; info: { frame: number; totalFrames: number; timeMs: number } }
    | undefined

  const logReplayExportProgress = async (
    gameId: string,
    info: { frame: number; totalFrames: number; timeMs: number },
  ) => {
    const percent =
      info.totalFrames > 0 ? ` ${((info.frame / info.totalFrames) * 100).toFixed(1)}%` : ''

    let sizePart = ''
    try {
      const stats = await fsPromises.stat(replayExportTimelineOutputPath())
      sizePart = ` timeline=${formatBytes(stats.size)}`
    } catch (err: any) {
      if (err?.code !== 'ENOENT') {
        sizePart = ' timeline=unavailable'
      }
    }

    logReplayExport(
      `progress gameId=${gameId} frame=${info.frame}/${info.totalFrames || '?'} time=${formatReplayExportTime(info.timeMs)}${percent}${sizePart}`,
    )
  }

  const scheduleReplayExportProgressLog = (
    gameId: string,
    info: { frame: number; totalFrames: number; timeMs: number },
  ) => {
    replayExportPendingProgress = { gameId, info }
    if (replayExportProgressLogInFlight) {
      return
    }

    replayExportProgressLogInFlight = true
    Promise.resolve()
      .then(async () => {
        while (replayExportPendingProgress) {
          const next = replayExportPendingProgress
          replayExportPendingProgress = undefined
          await logReplayExportProgress(next.gameId, next.info)
        }
      })
      .finally(() => {
        replayExportProgressLogInFlight = false
      })
  }
  ...
  activeGameManager
    .on('gameStatus', status => {
      ...
      if (replayExportLaunchArgs.enabled && status.isReplay) {
        const extra =
          status.extra === null || status.extra === undefined
            ? ''
            : ` extra=${typeof status.extra === 'string' ? status.extra : JSON.stringify(status.extra)}`
        logReplayExport(`status=${status.state} gameId=${status.id}${extra}`)

        if (status.state === 'finished') {
          if (replayExportQuitTimer) {
            clearTimeout(replayExportQuitTimer)
          }
          replayExportQuitTimer = setTimeout(() => {
            logReplayExport('completed, exiting app')
            app.quit()
          }, 2000)
          ...
          replayExportQuitTimer = setTimeout(() => {
            logReplayExport('error, exiting app')
            app.exit(1)
          }, 500)
        }
      }
    })
    .on('replayProgress', (gameId, info) => {
      if (!replayExportLaunchArgs.enabled) {
        return
      }
      scheduleReplayExportProgressLog(gameId, info)

      if (info.totalFrames > 0 && info.frame >= info.totalFrames) {
        if (!replayExportTerminalFrameTimer) {
          logReplayExport(`terminal frame reached for ${gameId}, sending quit`)
          replayExportTerminalFrameTimer = setTimeout(() => {
            replayExportTerminalFrameTimer = undefined
            activeGameManager.clearGameConfig(gameId)
            if (replayExportQuitTimer) {
              clearTimeout(replayExportQuitTimer)
            }
            replayExportQuitTimer = setTimeout(() => {
              logReplayExport('terminal-frame shutdown complete, exiting app')
              app.quit()
            }, 1500)
          }, 250)
        }
      } else if (replayExportTerminalFrameTimer) {
        clearTimeout(replayExportTerminalFrameTimer)
        replayExportTerminalFrameTimer = undefined
      }
    })
    ...
    .on('gameResult', info => {
      ...
      if (replayExportLaunchArgs.enabled) {
        logReplayExport(
          `result gameId=${info.gameId} time=${formatReplayExportTime(info.time)} rawTimeMs=${info.time}`,
        )
      }
    })
    .on('replaySaved', (gameId, replayPath) => {
      ...
      if (replayExportLaunchArgs.enabled) {
        logReplayExport(`replay-saved gameId=${gameId} path=${replayPath}`)
      }
    })
...
async function createWindow() {
  ...
  if (!process.argv.includes('--hidden') && !replayExportLaunchArgs.enabled) {
...
app.on('ready', () => {
  if (replayExportLaunchArgs.enabled) {
    logReplayExport(
      `starting speed=${replayExportLaunchArgs.targetMultiplier ?? 128} disableRender=${replayExportLaunchArgs.disableRender ?? true} disableAudio=${replayExportLaunchArgs.disableAudio ?? true}`,
    )
  }
  const localSettingsPromise = createLocalSettings()
  const scrSettingsPromise = createScrSettings()
  const programRegistrationPromise = replayExportLaunchArgs.enabled
    ? Promise.resolve()
    : registerCurrentProgram()
