...
BW Forge currently has no winner, loser, result, victory/defeat, team/force, map-dimension, replay-seed, or explicit start/end-frame contract. Legacy analysis manifests retain their historical nullable map field, while Corpus v2 derives `replays.map_name` from the canonical replay header. Result is not inferred from replay termination.
...
| Map name | The unit timeline does not serialize it and the legacy analyzer still writes `"map": null`. Corpus v2 separately loads the SHA-verified canonical replay through pinned screp v1.13.4, preferring nonblank `MapData.Name` and falling back to `Header.Map`. | Immutable legacy/top-level manifests may remain null. The accepted nonblank decoded string is stored in nullable SQLite `replays.map_name`; map discovery/filtering, replay cards, MCP, and regenerated static indexes consume that derived field. No bwsim initialization, simulation stepping, or analysis is required. |
...
- Map plumbing exists in SQLite, filters, cards, and MCP. Registration and explicit backfill derive it from canonical replay bytes through screp; rows that cannot be decoded remain `unknown`.
...
2. **Fields generated outside ShieldBattery telemetry:** replay SHA-256, source filename/path and copied replay provenance come from the original replay and CLI; player race and matchup are Python inference; duration is Python's conversion of the final snapshot frame. Corpus v2 map name comes independently from the canonical replay header; legacy manifests remain unchanged and may contain null.
...
6. **Currently missing or intentionally null:** map dimensions, teams/forces, explicit replay start/end frame, winner, loser, result, victory/defeat state, disconnect/leave reason, and replay seed. These are absent rather than partially reliable, except that termination can indirectly shorten the data.
7. **Replacement blocker assessment:** no metadata/result field appears to block replacement. The only simulator-sensitive compatibility obligation in this area is matching the last emitted frame/EOF behavior (plus owner IDs/names if they are not supplied by a replay parser). Result support would be a new feature, not a ShieldBattery-parity requirement. Exact map-name legacy-code-page fidelity, map dimensions, teams, and seed remain outside the current contract.
