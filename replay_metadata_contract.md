...
| Map name | The unit timeline does not serialize it and the legacy analyzer still writes `"map": null`. Corpus v2 separately loads the SHA-verified canonical replay and reads `replayHeader().mapName` at frame zero through the shared headless-bwsim metadata runtime. | Immutable legacy/top-level manifests may remain null. The accepted nonblank decoded string is stored in nullable SQLite `replays.map_name`; map discovery/filtering, replay cards, MCP, and regenerated static indexes consume that derived field. No simulation stepping or analysis is required. |
